...
-   `cloud`: Uploads your assets to Roblox. This will generate a `truffle.lock.toml` file which you should commit to source control. This is the default target.
...
Asphalt is configured with a project file called `truffle.toml`. It is required for the program to run.
...
Make sure that you select an appropriate IP and that your API key is under the Creator (user, or group) that you've defined in `truffle.toml`.
